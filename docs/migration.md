...
- **Testable.** 52 unit tests, ~200ms to run.
...
| No linter / formatter | ESLint 9 flat config + Prettier 3 |
...
- **Pure functions** (`src/game/board.ts`): `findMatches`, `applyGravity`, `refillEmpty`, etc. These take a grid in, return a grid (or a set of cells) out. No side effects, no React, no DOM.
...
The v1 game was effectively the same shape, but everything was in one closure and you couldn't reach in. Now the engine is a unit you can construct in a test, feed scenarios to, and assert on.
...
That's the entire surface. Everything else inside the engine is private.
...
In v2, we keep the stable `id`, but use it as React's `key`. That way React mounts each gem once and reuses the same DOM node across renders, and the CSS transition on `left` / `top` interpolates between the old and new positions.
...
In v2, the engine exposes `state.poppedGems: Set<number>` (ids of gems currently in pop animation) and `state.spawned: SpawnedGem[]` (gems that should render above the board so they animate in). The Board component reads these and adds the right CSS class / positions.
...
1. **Testability.** We can now write 50+ unit tests and run them in 200ms. In v1, any logic change required manual playthrough testing.
2. **Maintenance.** Adding a feature (e.g. a new special) in v1 meant carefully editing a 1500-line IIFE. In v2, it's a new function in `board.ts` + a test + a small UI change.
3. **HMR.** Vite's dev server reloads the page on save in milliseconds. The v1 file required a hard refresh.
4. **Tooling.** ESLint, Prettier, TypeScript — all things the v1 file didn't have.

The tradeoff is build complexity (Node + npm + bundler). For a personal toy match-3, it's probably overkill. For something you intend to extend and maintain, it's worth it.
...
After the initial v2 release we swapped the lint + format toolchain from **ESLint 10 + Prettier 3** to **Biome 2.5**, in the same commit as the TypeScript 5.9 → 7.0 bump.
...
- `typescript-eslint` relies on the TypeScript programmatic API, which got a major rewrite in TS 7.0 and doesn't stabilize until 7.1 (expected October 2026).
- Biome is Rust-based, doesn't use the TypeScript API at all — it parses TS syntax directly. So it works fine with TS 7.
- Side benefit: lint + format in one tool, the whole check runs in ~15ms (vs ~1.5s for ESLint + Prettier on the same codebase).
...
- ESLint plugin ecosystem. We were using `react-hooks` (exhaustive deps) and `react-refresh` (only-export-components). Biome's recommended ruleset has a `useExhaustiveDependencies` rule that does the same thing. We didn't use `react-refresh`'s rule in a way that mattered.

What we kept:

- `noUncheckedIndexedAccess` in `tsconfig.app.json` — the strict grid access pattern that makes the engine safe.
- The "no `!` non-null assertion" rule is **off** in Biome. We use `!` heavily in `board.ts` for grid access after bounds checks; it's intentional, and Biome's default rule would have us refactor every call site. (The eslint config also didn't have `no-non-null-assertion`.)
